...
from lib_config.vo import CommonEncoder, CommonDecoder
from . import REDIS
...
class BaseCache(StrictRedis):
    ...
    def set_json(self, key, value):
        return self.set(key, dumps(value, cls=CommonEncoder))

    def setex_json(self, key, expires, value):
        return self.setex(key, expires, dumps(value, cls=CommonEncoder))

    def get_json(self, key, default=None):
        v = self.get(key)
        return (loads(v.decode(), cls=CommonDecoder)
    ...
    def hset_json(self, key, field, value):
        return self.hset(key, field, dumps(value, cls=CommonEncoder))
    ...
    def hmset_json(self, key, value):
        return self.hmget(key, {k: dumps(v, cls=CommonEncoder)
                                for k, v in value.items()})

    def hgetall_json(self, key):
        return {k.decode(): loads(v.decode(), cls=CommonDecoder)
                for k, v in self.hgetall(key).items()}
    ...
    def lpush_json(self, key, *values):
        ...
        return self.lpush(key, *[dumps(v, cls=CommonEncoder) for v in values])

    def rpush_json(self, key, *values):
        if not values:
            return 0

        return self.rpush(key, *[dumps(v, cls=CommonEncoder) for v in values])

    def rpop_json(self, key, default=None):
        v = self.rpop(key)
        return (loads(v.decode(), cls=CommonDecoder)
                if v is not None else default)
    ...
    def brpop_json(self, key, timeout=0):
        ...
        return dumps(self.brpop(key, timeout), cls=CommonEncoder)

    def lrange_json(self, key, start, stop):
        values = self.lrange(key, start, stop)
        return [loads(v.decode(), cls=CommonDecoder) for v in values]
    ...
    def sadd_json(self, key, *values):
        ...
        return self.sadd(key, *[dumps(v, cls=CommonEncoder) for v in values])

    def srem_json(self, key, *values):
        if not values:
            return 0

        return self.srem(key, *[dumps(v, cls=CommonEncoder) for v in values])
    ...
    def srandmember_json(self, key, limit=None):
        values = self.srandmember(key, limit)
        if limit is None:
            return loads(values.decode(), cls=CommonDecoder)
        else:
            return [loads(v.decode(), cls=CommonDecoder) for v in values]
    ...
    def zadd_json(self, key, *args, **kwargs):
        ...
        for i, v in enumerate(args):
            if i % 2 == 1:
                args[i] = dumps(v, cls=CommonEncoder)

        n_kwargs = {}
        for k, v in kwargs.items():
            n_kwargs[k] = dumps(v, cls=CommonEncoder)
...
class RedisClusterPool(StrictRedisCluster):
    def __init__(self, alias):
        super(RedisClusterPool, self).__init__(startup_nodes=REDIS[alias]["startup_nodes"], decode_responses=True)

...
def create_mongodb_index(db_alias, coll=None, drop_indexes=False,
    ...
    from model.mongo.index import MONGODB_INDEXES
    global MONGODB_INDEXES
    ...
    if db_alias is not None:
        MONGODB_INDEXES = {k: v for k, v in MONGODB_INDEXES.items()
                           if k == db_alias}
        if coll is not None:
            MONGODB_INDEXES = {k: {k1: v1 for k1, v1 in v.items()
                                   if k1 == coll}
                               for k, v in MONGODB_INDEXES.items()}

    for db_alias, colls in MONGODB_INDEXES.items():
        db = get_mongo_database(db_alias)
        logger.info("begin process db {} ...".format(db.name))

        for coll, indexes in colls.items():
            coll = db[coll]
            logger.info(

...
from lib_config.mongodb import MONGODB
from lib_config.exception import CommonException
...
class BaseMongoModel(Collection):
    ...
    def modify(self, spec, document, multi=False, upsert=False, validate=True,
               sort=None, return_document=None, unset_empty=True):
        """部分更新文档
        ...
        if not doc:
            if return_document == ReturnDocument.AFTER or return_document == ReturnDocument.BEFORE:
                return self.find_one(spec)
            else:
                return 0
        if '$set' in doc:
            doc['$set']['update_time'] = datetime.utcnow()
        else:
            doc['$set'] = {'update_time': datetime.utcnow()}
        if multi:
    ...
    def _filter_none_value(doc):
        if not isinstance(doc, dict):
            raise CommonException("doc should be a dict")
    ...
    def _validate(cls, doc, required=True):
        ...
        for k, v in doc.items():
            ...
            if k not in cls._fields:
                raise CommonException("unexpected field '{}'".format(k))
        ...
        if required:
            ...
            for v in fields:
                if v not in doc:
                    raise CommonException("field '{}' is required".format(v))
